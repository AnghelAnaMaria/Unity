...
{//clasa care imi defineste Tilemap pt WFC
    public class InputReader : IInputReader<TileBase>
    {
        ...
        public IValue<TileBase>[][] ReadInputToGrid()
        {
            ...
            TileBaseValue[][] gridOfValues = null;//cream un 2D array cu instante TileBaseValue
            if (grid != null)
            {
            ...
            for (int row = 0; row < grid.Length; row++)
            {
                ...
                {
                    gridOfValues[row][col] = new TileBaseValue(grid[row][col]);//Aici ia fiecare TileBase și îl „împachetează” într-o instanță de TileBaseValue.  Nu facem o copie a sprite-ului sau a datelor tile-ului, ci doar păstrăm aceeași referință, dar într-un obiect wrapper care implementează IValue<TileBase>.
                }
            }
        ...
        private TileBase[][] ReadInputTileMap()//Facem rost de informatiile Tilemap-ului din scena
        {
            InputImageParameters imageParameters = new InputImageParameters(inputTilemap);//aici am informatiile Tilemap-ului din scena
            return CreateTileBaseGrid(imageParameters);
        }

        private TileBase[][] CreateTileBaseGrid(InputImageParameters imageParameters)//cream in memorie un Tilemap folosind informatiile din scena
        {
